...
void FDPlate::setup(float sampRate, FDPlate::PlateParameters plateParams)
{
    ...
    u = std::make_unique<float[]>(ss);
    u1 = std::make_unique<float[]>(ss);
    u2 = std::make_unique<float[]>(ss);
...
//    updateSides();
//    updateCorners();
    std::swap(dummyptr, u2); std::swap(u2, u1); std::swap(u1, u); std::swap(u,dummyptr);
...
//==============================================================================
int FDPlate::getGridSize()
{
    return Nx*Ny;
}

float* FDPlate::getCurrentState()
{
    return u.get();
}
